...
function App() {
  ...
  if(answerItem.length > 3) {
     answerItem.pop();
     console.log(answerItem, 'heeeeeeee');
  }
  ...
  return (
    <StyledApp>
      ...
      <Menu style={{justifyContent: 'flex-end'}} theme="dark" mode="horizontal" defaultSelectedKeys={['2']}>
        ...
        <Menu.Item key="subract">Subtract</Menu.Item>
        <Menu.Item key="multiply">Multiply</Menu.Item>
        <Menu.Item key="divide">Divide</Menu.Item>
      </Menu>
    </Header>
      <ToastContainer
        position="top-center"
            ...
            <div className="app__card-inner">
              {
                answerItem.sort(() => .5 - Math.random()).map(item => (
                  <div style={{backgroundColor: `${colors.sort( () => .5 - Math.random())[0]}`}} key={item} onClick={() => handleAnswer(item)}>
                    <h1>{item}</h1>
                  </div>
                ))
              }
